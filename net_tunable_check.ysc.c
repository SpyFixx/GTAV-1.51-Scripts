...
bool func_1(var uParam0, var uParam1)
{
	...
	if (uParam0->f_2)
	{
		if (MISC::ABSI(NETWORK::IS_TIME_EQUAL_TO(MISC::GET_GAME_TIMER(), uParam0->f_1)) >= uParam0->f_3)
		{
			func_5();
		}
	}
			...
			while (iVar2 <= iVar11)
			{
				if (iVar2 < iVar10)
				{
					...
					if (iVar2 >= Var3.f_4 && iVar2 != 27)
					{
						...
						{
							if (NETWORK::_NETWORK_REGISTER_TUNABLE_INT_HASH(iVar12, Var3.f_5, &fVar0))
							{
							...
							else if (NETWORK::_NETWORK_ALLOCATE_TUNABLES_REGISTRATION_DATA_MAP(iVar12, Var3.f_5, &iVar1))
							{
...
bool func_17(int iParam0, var uParam1)
{
