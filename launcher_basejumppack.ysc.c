...
void func_41()
{
	...
	func_42(&vVar0, &uVar3);
	vVar4 = { 0f, 0f, uVar3 };
	fVar7 = PED::CREATE_SYNCHRONIZED_SCENE(vVar0, vVar4, 2);
	ENTITY::PLAY_SYNCHRONIZED_ENTITY_ANIM(iLocal_297, fVar7, "puton_parachute_bag", "oddjobs@basejump@ig_15", 1000f, -1000f, 1, 1000f);
	if (!ENTITY::IS_ENTITY_DEAD(PLAYER::PLAYER_PED_ID(), false))
	{
		TASK::CLEAR_PED_TASKS(PLAYER::PLAYER_PED_ID());
		TASK::TASK_SYNCHRONIZED_SCENE(PLAYER::PLAYER_PED_ID(), fVar7, "oddjobs@basejump@ig_15", "puton_parachute", 1000f, -1000f, 0, 0, 1000f, 0);
	}
	AUDIO::PLAY_SOUND_FRONTEND(-1, "Grab_Parachute", "BASEJUMPS_SOUNDS", true);
	iVar8 = CAM::CREATE_CAM("DEFAULT_ANIMATED_CAMERA", false);
	CAM::PLAY_SYNCHRONIZED_CAM_ANIM(iVar8, fVar7, "puton_parachute_cam", "oddjobs@basejump@ig_15");
	CAM::SET_CAM_ACTIVE(iVar8, true);
	CAM::RENDER_SCRIPT_CAMS(true, false, 3000, true, false, 0);
	while (PED::GET_SYNCHRONIZED_SCENE_PHASE(fVar7) < 0.6f)
	{
		SYSTEM::WAIT(0);
	}
...
float func_65(struct<2> Param0, var uParam2, struct<2> Param3, var uParam5)
{
